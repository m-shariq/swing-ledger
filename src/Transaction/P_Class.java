...
public class P_Class {
    ...
    P_Class(){}
    P_Class(int ID,String SERIAL,String PARTY,String QTY,String COST,String TOTAL,String PRNO,String BATCH,String DATE,String EXPIRY,String ITEM,String ITEMDESC,String UNIT,String DIST)
    {
        this.id=ID;
        this.serial=SERIAL;
        this.party=PARTY;
    ...
    public boolean purchaseTypeQuery(String queryType, P_Class type){
        PreparedStatement ps;
        if(queryType.equals("add")){
            try {
                ps=CONNECTION.getConnection().prepareStatement("INSERT INTO `purchased`(`SERIAL`, `PARTY`, `QTY`, `COST`, `TOTAL`, `PRNO`, `BATCH`, `DATE`, `EXPIRY`, `ITEM`, `ITEM_DESC`, `UNIT`, `DIST`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            try {
                ps=CONNECTION.getConnection().prepareStatement("UPDATE `purchased` SET `SERIAL`=?,`PARTY`=?,`QTY`=?,`COST`=?,`TOTAL`=?,`PRNO`=?,`BATCH`=?,`DATE`=?,`EXPIRY`=?,`ITEM`=?,`ITEM_DESC`=?,`UNIT`=?,`DIST`=? WHERE `ID`=?");
                 ps.setString(1, type.getSerial());
                 ps.setString(2, type.getParty());
                 ps.setString(3, type.getQty());
                 ps.setString(4, type.getCost());
            ...
            try {
                ps=CONNECTION.getConnection().prepareStatement("DELETE FROM `purchased` WHERE `ID`=?");
