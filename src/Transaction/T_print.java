...
public class T_print {
    ...
    T_print(){}
    T_print(int ID,String SERIAL,String BILTY,String DATE,String TRANS,String TOTAL,String STATUS)
    {
        this.id=ID;
        this.serial=SERIAL;
        this.bilty=BILTY;
    ...
    public boolean printTypeQuery(String queryType, T_print type){
        PreparedStatement ps;
        if(queryType.equals("POST")){
            try {
                ps=CONNECTION.getConnection().prepareStatement("INSERT INTO `purchase_print`(`SERIAL`, `BILTY`, `TRANS`, `DATE`, `TOTAL`, `STATUS`) VALUES (?,?,?,?,?,?)");
            ...
            try {
                ps=CONNECTION.getConnection().prepareStatement("DELETE FROM `purchase_print` WHERE `SERIAL`=?");
                    ps.setString(1, type.getSerial());
                 return (ps.executeUpdate()>0);
                } 
            catch (SQLException ex) {
